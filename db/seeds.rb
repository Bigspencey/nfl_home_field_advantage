...
# Seeds initial season data
Season.populate_data

# Seeds Seasons with offense statistics
CSV.foreach("lib/assets/total_offense.csv") do |row|
	season = (Season.includes(:team).select do |season|
		season.team.name == row[0] && season.year == row[2].to_i
	end).first
	season.update_attributes(avg_offense: row[1])
end

# Seeds Seasons with defense statistics
